...
# We are defining our tokenizer here, in case we want to use a different tokenizer in the future
...
def extractor(input: pd.DataFrame, token: str, lower: bool=False, window: int=2, pos_regex: str='.?') -> [dict]:
    ...
    for i in input.index:
        ...
        for tuple_index in range(0, len(tags)):
            ...
            # then we trigger.
            if re.match(pos_regex, tag_tuple[1]) and re.match(token, tag_tuple[0]):
                ...
                if index - 1 >= 0:
                    token_neg = tags[index-1][0] + " " + tags[index][0]
                    token_features.append(token_neg)
                try:
                    token_pos = tags[index][0] + " " + tags[index+1][0]
                    token_features.append(token_pos)
                except:
                    pass
                if index - 1 >= 0:
                    try:
                        token_all = tags[index-1][0] + " " + tags[index][0] + " " + tags[index+1][0]
                        token_features.append(token_all)
                    except:
                        pass
                # Now we iterate according to window size.
                for e in range(2, window+1):
                    if index - e >= 0:
                        token_neg = tags[index - e][0] + " " + token_neg
                        token_features.append(token_neg)
                    try:
                        token_pos = token_pos + " " + tags[index + e][0]
                        token_features.append(token_pos)
                    except:
                        pass
                    if index - e >= 0:
                        try:
                            token_all= (tags[index - e][0] + " " + token_all + " " + tags[index + e][0])
                            token_features.append(token_all)
                        except:
                            pass
                # Now we repeat the same process as above for the POS tags
                if index - 1 >= 0:
                    token_neg = tags[index - 1][1] + " " + tags[index][1]
                    pos_features.append(token_neg)
                try:
                    token_pos = tags[index][1] + " " + tags[index + 1][1]
                    pos_features.append(token_pos)
                except:
                    pass
                if index - 1 >= 0:
                    try:
                        token_all = tags[index - 1][1] + " " + tags[index][1] + " " + tags[index + 1][1]
                        pos_features.append(token_all)
                    except:
                        pass
                # Now we iterate according to window size.
                for e in range(2, window + 1):
                    if index - e >= 0:
                        token_neg = tags[index - e][1] + " " + token_neg
                        pos_features.append(token_neg)
                    try:
                        token_pos = token_pos + " " + tags[index + e][1]
                        pos_features.append(token_pos)
                    except:
                        pass
                    if index - e >= 0:
                        try:
                            token_all = (tags[index - e][1] + " " + token_all + " " + tags[index + e][1])
                            pos_features.append(token_all)
                        except:
                            pass
            else:
            ...
            if token_features:
                all_features = [token_features,pos_features]
                ...
                temp_dic = {"Id": token_id, "token": tags[index][0], "features": all_features}
...
def export(features: [dict], filename: str, encod= str("utf-8")):
...
def export_multiple(list_of_features: [[dict]], filename: str, encod= str("utf-8")):
...
def main() -> None:
    ...
    if args.file and not args.dir:
        ...
        if args.uncased:
            tokens = extractor(file, token=args.token_regex, pos_regex=args.pos_regex, lower=True, window=args.window)
        else:
            tokens = extractor(file, lower=False, window=args.window, token=args.token_regex, pos_regex=args.pos_regex)
        export(features=tokens,filename=args.outputfile,encod=args.encoding)
        print("success")
